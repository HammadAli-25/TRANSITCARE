...
# --------------------------------------------------
# AUTOMATIC 10-DAY CUSTOMER-CARE ESCALATION
# --------------------------------------------------

NORMAL_ESCALATION_DAYS = 10

def _parse_issue_date(date_text):
...
def auto_escalate_overdue_complaints():
    conn = None
    try:
        conn = get_db(); c = conn.cursor(); now = datetime.now()
        c.execute("SELECT id, user_id, date FROM issues WHERE status != 'Resolved'")
        for issue in c.fetchall():
            parsed = _parse_issue_date(issue["date"])
            if not parsed or (now.date() - parsed.date()).days < NORMAL_ESCALATION_DAYS:
                continue
            c.execute("SELECT id FROM chat_escalations WHERE issue_id=? LIMIT 1", (issue["id"],))
            if c.fetchone():
                continue
            days = max(0, (now.date() - parsed.date()).days)
            c.execute("""INSERT INTO chat_escalations (user_id, issue_id, reason, message, status, created_at) VALUES (?, ?, ?, ?, ?, ?)""", (issue["user_id"], issue["id"], "Normal issue unresolved for 10 or more days", f"Automatically escalated because this complaint has remained unresolved for {days} days.", "Open", now.strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
    except Exception as exc:
        print("Automatic escalation check failed:", exc)
        if conn: conn.rollback()
    finally:
        if conn: conn.close()

def _automatic_escalation_worker():
    while True:
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        #
        # Emergency detection is based on the complaint
        # description entered during submission. If any
        # configured emergency keyword is found, the complaint
        # is immediately sent to Customer Care without waiting
        # for the chatbot or the normal 10-day escalation rule.
        # --------------------------------------------------
        emergency_keywords = [
            "emergency",
            "accident",
        ...
        if is_emergency:
            escalation_message = (
                "Automatically escalated after complaint submission because "
                "an emergency keyword was detected in the description: "
                f"{description}"
            )
            ...
            """, (
                session["user_id"],
                issue_id,
                "Emergency issue - automatic escalation after submission",
                escalation_message,
                "Open",
                current_date
            ))
        ...
        if is_emergency:
            session["success_msg"] = (
                "Complaint submitted successfully and automatically "
                "escalated to Customer Care because an emergency keyword "
                "was detected."
            )
        else:
            session["success_msg"] = (
...
def authority():
    ...
    # All complaints
    # Emergency complaints that were automatically escalated at
    # submission are shown in Customer-care escalations instead of
    # this table. Other complaints remain unchanged.
    c.execute("""
        SELECT *
        FROM issues i
        WHERE NOT EXISTS (
            ...
            WHERE ce.issue_id = i.id
              AND ce.reason = 'Emergency issue - automatic escalation after submission'
        )
        ORDER BY i.id DESC
    """)
...
def update_escalation():
    ...
    c.execute("SELECT issue_id FROM chat_escalations WHERE id=?", (escalation_id,))
    escalation_row = c.fetchone()
    if escalation_row and escalation_row["issue_id"]:
        if status == "Resolved":
            c.execute("UPDATE issues SET status='Resolved' WHERE id=?", (escalation_row["issue_id"],))
        elif status == "In Progress":
            c.execute("UPDATE issues SET status='In Review' WHERE id=? AND status='Resolved'", (escalation_row["issue_id"],))
        elif status == "Open":
            c.execute("UPDATE issues SET status='Pending' WHERE id=? AND status='Resolved'", (escalation_row["issue_id"],))
...
def update_status():
    ...
    if status == "Resolved":
        c.execute("UPDATE chat_escalations SET status='Resolved' WHERE issue_id=?", (issue_id,))
            ...
            if not latest_issue:
                ...
                if issue_status == "Resolved":
                    ...
                    if existing_escalation:
                        ...
                        days_since_report = 0
                        parsed_issue_date = _parse_issue_date(issue_date_text)
...
if __name__ == "__main__":
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
        threading.Thread(target=_automatic_escalation_worker, daemon=True).start()
    app.run(debug=True)